...
void* iperf_server_worker_run(void* s)
{
...
{
retry:
    if ((test->listener = netannounce(test->settings->domain, Ptcp,
...
int iperf_accept(struct iperf_test* test)
{
    ...
    if (test->ctrl_sck == -1) {
        ...
        if ((opt = test->settings->snd_timeout)) {
            if (setsockopt(s, IPPROTO_TCP, TCP_USER_TIMEOUT, &opt,
                           sizeof(opt)) < 0) {
	 ...
	 */
        if (Nwrite(s, (char*)&rbuf, sizeof(rbuf), Ptcp) < 0) {
            if (test->debug)
                printf(
                    "failed to send ACCESS_DENIED to an unsolicited connection request during active test\n");
        }
        else {
            if (test->debug)
                printf(
                    "successfully sent ACCESS_DENIED to an unsolicited connection request during active test\n");
        }
        close(s);
    }
...
int iperf_handle_message_server(struct iperf_test* test)
{
    ...
    if ((rval = Nread(test->ctrl_sck, (char*)&test->state, sizeof(signed char),
                      Ptcp)) <= 0) {
        if (rval == 0) {
            iperf_err(test,
...
static void server_timer_proc(TimerClientData client_data,
...
static void server_stats_timer_proc(TimerClientData client_data,
                                    struct iperf_time* nowP)
{
    struct iperf_test* test = client_data.p;
...
static void server_reporter_timer_proc(TimerClientData client_data,
                                       struct iperf_time* nowP)
{
    struct iperf_test* test = client_data.p;
...
static int create_server_timers(struct iperf_test* test)
{
    ...
    if (test->duration != 0) {
        test->done = 0;
        test->timer =
            tmr_create(&now, server_timer_proc, cd,
    ...
    if (test->stats_interval != 0) {
        test->stats_timer = tmr_create(&now, server_stats_timer_proc, cd,
                                       test->stats_interval * SEC_TO_US, 1);
    ...
    if (test->reporter_interval != 0) {
        test->reporter_timer = tmr_create(&now, server_reporter_timer_proc, cd,
...
static void server_omit_timer_proc(TimerClientData client_data,
                                   struct iperf_time* nowP)
{
    struct iperf_test* test = client_data.p;
...
static int create_server_omit_timer(struct iperf_test* test)
{
    struct iperf_time now;
    TimerClientData cd;
    ...
    if (test->omit == 0) {
        ...
        test->omitting = 1;
        cd.p = test;
        test->omit_timer = tmr_create(&now, server_omit_timer_proc, cd,
                                      test->omit * SEC_TO_US, 0);
...
static void cleanup_server(struct iperf_test* test)
{
    ...
    SLIST_FOREACH(sp, &test->streams, streams)
    {
        ...
        if (rc != 0 && rc != ESRCH) {
            ...
            errno = rc;
            iperf_err(test, "cleanup_server in pthread_cancel - %s",
        ...
        if (rc != 0 && rc != ESRCH) {
            ...
            errno = rc;
            iperf_err(test, "cleanup_server in pthread_join - %s",
                      iperf_strerror(i_errno));
        }
...
int iperf_run_server(struct iperf_test* test)
{
    ...
    if (test->json_output) {
        cJSON_AddItemToObject(test->json_start, "version",
                              cJSON_CreateString(version));
        cJSON_AddItemToObject(test->json_start, "system_info",
                              cJSON_CreateString(get_system_info()));
    }
    ...
    while (test->state != IPERF_DONE) {
        ...
        if (test->state ==
            IPERF_START) { // In idle mode server may need to restart
            if (timeout == NULL && test->settings->idle_timeout > 0) {
                ...
                timeout = &used_timeout;
                 ...
                 SENDER) { // In non-reverse active mode server ensures data is received
            timeout_us = -1;
            if (timeout != NULL) {
                ...
                if (test->state == IPERF_START) {
                    if (test->settings->idle_timeout > 0 &&
                        ...
                        if (iperf_get_test_one_off(test)) {
                            if (test->debug)
                                printf(
                                    "No connection request was received for %d sec in one-off mode; exiting.\n",
                                    test->settings->idle_timeout);
                            exit(0);
                        }
                 ...
                 */
                    ...
                    if (test->blocks_received == last_receive_blocks) {
                        ...
                        if (iperf_get_verbose(test))
                            iperf_err(
                                test,
                                "Server restart (#%d) during active test due to idle timeout for receiving data",
        ...
        if (result > 0) {
            ...
            if (test->state == CREATE_STREAMS) {
				 ...
				 */
                                    if (errno == ENOENT) {
                                        warning(
                                            "TCP congestion control algorithm not supported");
                                    }
                                    else {
                                        saved_errno = errno;
                                        close(s);
                            ...
                            }
                            {
                                ...
                                int rc;
                                rc = getsockopt(s, IPPROTO_TCP, TCP_CONGESTION,
                                                ca, &len);
                ...
                if (rec_streams_accepted == streams_to_rec &&
                    ...
                    if (test->protocol->id != Ptcp) {
                        ...
                        if (test->no_delay || test->settings->mss ||
                            ...
                            close(test->listener);
                            test->listener = -1;
                            if ((s = netannounce(test->settings->domain, Ptcp,
                                                 test->bind_address,
                                                 test->bind_dev,
                                                 test->server_port)) < 0) {
                                cleanup_server(test);
                    ...
                    if (test->settings->bitrate_limit > 0 &&
                        ...
                        if (iperf_get_verbose(test))
                            iperf_err(
                                test,
                                "Client total requested throughput rate of %" PRIu64
                                " bps exceeded %" PRIu64 " bps limit",
                    ...
                    SLIST_FOREACH(sp, &test->streams, streams)
                    {
                        if (pthread_create(&(sp->thr), &attr,
                                           &iperf_server_worker_run, sp) != 0) {
        ...
        if (result == 0 || (timeout != NULL && timeout->tv_sec == 0 &&
                            timeout->tv_usec == 0)) {
            /* Run the timers. */
            iperf_time_now(&now);
            tmr_run(&now);
