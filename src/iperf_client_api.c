...
void* iperf_client_worker_run(void* s)
{
...
int iperf_create_streams(struct iperf_test* test, int sender)
{
    ...
    for (i = 0; i < test->num_streams; ++i) {
        ...
        if (test->protocol->id == Ptcp) {
            if (test->congestion) {
                if (setsockopt(s, IPPROTO_TCP, TCP_CONGESTION, test->congestion,
...
static void test_timer_proc(TimerClientData client_data,
...
static void client_stats_timer_proc(TimerClientData client_data,
                                    struct iperf_time* nowP)
{
    struct iperf_test* test = client_data.p;
...
static void client_reporter_timer_proc(TimerClientData client_data,
                                       struct iperf_time* nowP)
{
    struct iperf_test* test = client_data.p;
...
static int create_client_timers(struct iperf_test* test)
{
    ...
    if (test->duration != 0) {
        test->done = 0;
        test->timer = tmr_create(&now, test_timer_proc, cd,
                                 (test->duration + test->omit) * SEC_TO_US, 0);
        if (test->timer == NULL) {
            i_errno = IEINITTEST;
            return -1;
        }
    ...
    if (test->stats_interval != 0) {
        test->stats_timer = tmr_create(&now, client_stats_timer_proc, cd,
    ...
    if (test->reporter_interval != 0) {
        test->reporter_timer = tmr_create(&now, client_reporter_timer_proc, cd,
...
static void client_omit_timer_proc(TimerClientData client_data,
                                   struct iperf_time* nowP)
{
    struct iperf_test* test = client_data.p;
...
static int create_client_omit_timer(struct iperf_test* test)
{
    struct iperf_time now;
    TimerClientData cd;
    if (NULL == test) {
    ...
    if (test->omit == 0) {
        ...
        test->omitting = 1;
        cd.p = test;
        test->omit_timer = tmr_create(&now, client_omit_timer_proc, cd,
...
int iperf_handle_message_client(struct iperf_test* test)
{
    ...
    if ((rval = read(test->ctrl_sck, (char*)&test->state,
...
int iperf_connect(struct iperf_test* test)
{
    int opt;
    socklen_t len;
    ...
    if (test->ctrl_sck < 0)
        ...
        test->ctrl_sck = netdial(test->settings->domain, Ptcp,
                                 test->bind_address, test->bind_dev, 0,
    ...
    if ((opt = test->settings->snd_timeout)) {
        if (setsockopt(test->ctrl_sck, IPPROTO_TCP, TCP_USER_TIMEOUT, &opt,
                       sizeof(opt)) < 0) {
	 ...
	 */
        if (test->ctrl_sck_mss > 0 &&
            ...
            snprintf(
                str, sizeof(str),
                "UDP block size %d exceeds TCP MSS %d, may result in fragmentation / drops",
                test->settings->blksize, test->ctrl_sck_mss);
            warning(str);
        }
    }
...
int iperf_client_end(struct iperf_test* test)
{
    if (NULL == test) {
        iperf_err(NULL, "No test\n");
        return -1;
...
int iperf_run_client(struct iperf_test* test)
{
    ...
    while (test->state != IPERF_DONE) {
        ...
        result = select(test->max_fd + 1, &read_set,
                        (test->state == TEST_RUNNING && !test->reverse) ?
                            &write_set :
                            NULL,
                        NULL, timeout);
#else
    ...
    SLIST_FOREACH(sp, &test->streams, streams)
    {
        if (!sp->sender) {
            ...
            if (rc != 0 && rc != ESRCH) {
                i_errno = IEPTHREADCANCEL;
                errno = rc;
                iperf_err(test, "receiver cancel in pthread_cancel - %s",
                          iperf_strerror(i_errno));
                goto cleanup_and_fail;
            }
            rc = pthread_join(sp->thr, NULL);
            if (rc != 0 && rc != ESRCH) {
                i_errno = IEPTHREADJOIN;
                errno = rc;
                iperf_err(test, "receiver cancel in pthread_join - %s",
                          iperf_strerror(i_errno));
                goto cleanup_and_fail;
            }
            if (test->debug_level >= DEBUG_LEVEL_INFO) {
...
cleanup_and_fail:
    ...
    SLIST_FOREACH(sp, &test->streams, streams)
    {
        ...
        if (rc != 0 && rc != ESRCH) {
            i_errno = IEPTHREADCANCEL;
            errno = rc;
            iperf_err(test, "cleanup_and_fail in pthread_cancel - %s",
                      iperf_strerror(i_errno));
        }
        rc = pthread_join(sp->thr, NULL);
        if (rc != 0 && rc != ESRCH) {
            i_errno = IEPTHREADJOIN;
            errno = rc;
            iperf_err(test, "cleanup_and_fail in pthread_join - %s",
                      iperf_strerror(i_errno));
        }
        if (test->debug_level >= DEBUG_LEVEL_INFO) {
            iperf_printf(test, "Thread FD %d stopped\n", sp->socket);
    ...
    iperf_client_end(test);
    if (test->json_output) {
        cJSON_AddStringToObject(test->json_top, "error",
                                iperf_strerror(i_errno));
