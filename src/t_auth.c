...
#if defined(HAVE_SSL)
int test_authtoken(const char* authUser, const char* authPassword,
...
int main(int argc, char** argv)
{
    ...
    const char sha256Digest[] =
        "4816482f8b4149f687a1a33d61a0de6b611364ec0fb7adffa59ff2af672f7232"; /* echo -n "This is a SHA256 test." | shasum -a256 */
    ...
    const char base64EncodeCheck[] =
        "VGhpcyBpcyBhIEJhc2U2NCB0ZXN0Lg=="; /* echo -n "This is a Base64 test." | b64encode -r - */

    assert(Base64Encode((unsigned char*)base64String, strlen(base64String),
                        &base64Text) == 0);
    assert(strcmp(base64Text, base64EncodeCheck) == 0);
    assert(Base64Decode(base64Text, (unsigned char**)&base64Decode,
                   ...
                   EVP_PKEY* pubkey, EVP_PKEY* privkey)
{
    ...
    assert(encode_auth_setting(authUser, authPassword, pubkey, &authToken) ==
           0);
    assert(decode_auth_setting(0, authToken, privkey, &decodeUser,
                               &decodePassword, &decodeTime) == 0);
    ...
    assert(now - decodeTime <=
           1); /* shouldn't take more than a second to run */
