...
static int JSONStream_Output(struct iperf_test* test, const char* event_name,
                             cJSON* obj);
...
{
    fprintf(f, usage_longstr, DEFAULT_NO_MSG_RCVD_TIMEOUT,
            UDP_RATE / (1024 * 1024), DEFAULT_PACING_TIMER, DURATION,
...
int iperf_get_control_socket_mss(struct iperf_test* ipt)
{
...
uint64_t iperf_get_test_bitrate_limit(struct iperf_test* ipt)
{
    return ipt->settings->bitrate_limit;
}

double iperf_get_test_bitrate_limit_interval(struct iperf_test* ipt)
{
    return ipt->settings->bitrate_limit_interval;
}

int iperf_get_test_bitrate_limit_stats_per_interval(struct iperf_test* ipt)
{
    return ipt->settings->bitrate_limit_stats_per_interval;
}

uint64_t iperf_get_test_fqrate(struct iperf_test* ipt)
{
...
int iperf_get_test_idle_timeout(struct iperf_test* ipt)
{
...
struct iperf_time* iperf_get_test_rcv_timeout(struct iperf_test* ipt)
{
...
void iperf_set_test_reporter_interval(struct iperf_test* ipt,
                                      double reporter_interval)
{
    ipt->reporter_interval = reporter_interval;
}

void iperf_set_test_stats_interval(struct iperf_test* ipt,
...
void iperf_set_test_bitrate_limit_maximum(struct iperf_test* ipt,
                                          uint64_t total_rate)
{
    ipt->settings->bitrate_limit = total_rate;
}

void iperf_set_test_bitrate_limit_interval(struct iperf_test* ipt,
                                           uint64_t bitrate_limit_interval)
{
    ipt->settings->bitrate_limit_interval = bitrate_limit_interval;
}

void iperf_set_test_bitrate_limit_stats_per_interval(
    struct iperf_test* ipt, uint64_t bitrate_limit_stats_per_interval)
{
    ipt->settings->bitrate_limit_stats_per_interval =
        bitrate_limit_stats_per_interval;
}

void iperf_set_test_fqrate(struct iperf_test* ipt, uint64_t fqrate)
{
...
void iperf_set_test_server_port(struct iperf_test* ipt, int srv_port)
{
...
void iperf_set_test_repeating_payload(struct iperf_test* ipt,
...
void iperf_set_test_timestamp_format(struct iperf_test* ipt, const char* tf)
{
...
void iperf_set_on_new_stream_callback(struct iperf_test* ipt,
                                      void (*callback)())
{
    ipt->on_new_stream = callback;
}

void iperf_set_on_test_start_callback(struct iperf_test* ipt,
                                      void (*callback)())
{
    ipt->on_test_start = callback;
}

void iperf_set_on_test_connect_callback(struct iperf_test* ipt,
                                        void (*callback)())
{
    ipt->on_connect = callback;
}

void iperf_set_on_test_finish_callback(struct iperf_test* ipt,
                                       void (*callback)())
{
    ipt->on_test_finish = callback;
}

static void check_sender_has_retransmits(struct iperf_test* ipt)
{
    if (ipt->mode != RECEIVER && ipt->protocol->id == Ptcp &&
...
void iperf_set_test_server_hostname(struct iperf_test* ipt,
...
void iperf_set_test_get_server_output(struct iperf_test* ipt,
...
#if defined(HAVE_SSL)
void iperf_set_test_client_username(struct iperf_test* ipt,
                                    const char* client_username)
{
    ipt->settings->client_username = strdup(client_username);
}

void iperf_set_test_client_password(struct iperf_test* ipt,
                                    const char* client_password)
{
    ipt->settings->client_password = strdup(client_password);
}

void iperf_set_test_client_rsa_pubkey(struct iperf_test* ipt,
                                      const char* client_rsa_pubkey_base64)
{
    ipt->settings->client_rsa_pubkey =
        load_pubkey_from_base64(client_rsa_pubkey_base64);
}

void iperf_set_test_server_authorized_users(struct iperf_test* ipt,
                                            const char* server_authorized_users)
{
    ipt->server_authorized_users = strdup(server_authorized_users);
}

void iperf_set_test_server_skew_threshold(struct iperf_test* ipt,
                                          int server_skew_threshold)
{
    ipt->server_skew_threshold = server_skew_threshold;
}

void iperf_set_test_server_rsa_privkey(struct iperf_test* ipt,
                                       const char* server_rsa_privkey_base64)
{
    ipt->server_rsa_private_key =
        load_privkey_from_base64(server_rsa_privkey_base64);
}
#endif // HAVE_SSL

void iperf_set_test_bind_address(struct iperf_test* ipt,
                                 const char* bnd_address)
{
...
void iperf_set_test_udp_counters_64bit(struct iperf_test* ipt,
...
void iperf_set_dont_fragment(struct iperf_test* ipt, int dnf)
{
...
void iperf_on_test_start(struct iperf_test* test)
{
    if (test->json_output) {
        cJSON_AddItemToObject(
            ...
            iperf_json_printf(
                "protocol: %s  num_streams: %d  blksize: %d  omit: %d  duration: %d  bytes: %d  blocks: %d  reverse: %d  tos: %d  target_bitrate: %d bidir: %d fqrate: %d",
                ...
                (int64_t)test->bidirectional,
                (uint64_t)test->settings->fqrate));
    }
    else {
        if (test->verbose) {
            if (test->settings->bytes)
                iperf_printf(test, test_start_bytes, test->protocol->name,
                             test->num_streams, test->settings->blksize,
                             test->omit, test->settings->bytes,
                             test->settings->tos);
            else if (test->settings->blocks)
                iperf_printf(test, test_start_blocks, test->protocol->name,
                             test->num_streams, test->settings->blksize,
                             test->omit, test->settings->blocks,
                             test->settings->tos);
            else
                iperf_printf(test, test_start_time, test->protocol->name,
                             test->num_streams, test->settings->blksize,
                             test->omit, test->duration, test->settings->tos);
        }
    }
    if (test->json_stream) {
...
*/
static int mapped_v4_to_regular_v4(char* str)
{
    char* prefix = "::ffff:";
    int prefix_len;
...
void iperf_on_connect(struct iperf_test* test)
{
    ...
    if (test->json_output)
        cJSON_AddItemToObject(test->json_start, "timestamp",
                              iperf_json_printf("time: %s  timesecs: %d",
                                                now_str, (int64_t)now_secs));
    ...
    if (test->role == 'c') {
        if (test->json_output)
            cJSON_AddItemToObject(
                test->json_start, "connecting_to",
                iperf_json_printf("host: %s  port: %d", test->server_hostname,
                                  (int64_t)test->server_port));
        else {
            iperf_printf(test, report_connecting, test->server_hostname,
                         test->server_port);
            if (test->reverse)
        ...
        if (test->json_output)
            cJSON_AddItemToObject(test->json_start, "accepted_connection",
                                  iperf_json_printf("host: %s  port: %d", ipr,
                                                    (int64_t)port));
        else
    ...
    if (test->json_output) {
        ...
        if (test->protocol->id == SOCK_STREAM) {
            if (test->settings->mss)
                cJSON_AddNumberToObject(test->json_start, "tcp_mss",
                                        test->settings->mss);
            else {
 ...
 */
int iperf_parse_hostname(struct iperf_test* test, char* spec, char** p,
                         char** p1)
{
    struct in6_addr ipv6_addr;

    // Format is <addr>[%<device>]
    if ((*p = strtok(spec, "%")) != NULL && (*p1 = strtok(NULL, "%")) != NULL) {
        /*
        ...
        if (inet_pton(AF_INET6, *p, &ipv6_addr) == 1 &&
            ...
            if (test->debug) {
                iperf_printf(test,
                             "IPv6 link-local address literal detected\n");
            }
            return 0;
        }
        /*
...
int iperf_parse_arguments(struct iperf_test* test, int argc, char** argv)
{
    static struct option longopts[] = {
        ...
        { "server-bitrate-limit", required_argument, NULL,
        ...
        { "rsa-public-key-path", required_argument, NULL,
          OPT_CLIENT_RSA_PUBLIC_KEY },
        { "rsa-private-key-path", required_argument, NULL,
          OPT_SERVER_RSA_PRIVATE_KEY },
        { "authorized-users-path", required_argument, NULL,
          OPT_SERVER_AUTHORIZED_USERS },
        { "time-skew-threshold", required_argument, NULL,
          OPT_SERVER_SKEW_THRESHOLD },
#endif /* HAVE_SSL */
    ...
    blksize = 0;
    server_flag = client_flag = rate_flag = duration_flag = rcv_timeout_flag =
        snd_timeout_flag = 0;
#if defined(HAVE_SSL)
    ...
    while ((flag = getopt_long(
        ...
        case 'v':
            printf("%s (cJSON %s)\n%s\n%s\n", version, cJSON_Version(),
                   get_system_info(), get_optional_features());
            exit(0);
        case 's':
            if (test->role == 'c') {
        ...
        case OPT_SERVER_BITRATE_LIMIT:
            ...
            if (slash) {
                *slash = '\0';
                ...
                if (test->settings->bitrate_limit_interval !=
                        0 && /* Using same Max/Min limits as for Stats Interval */
                    (test->settings->bitrate_limit_interval < MIN_INTERVAL ||
                     test->settings->bitrate_limit_interval > MAX_INTERVAL)) {
        ...
        case OPT_IDLE_TIMEOUT:
            ...
            if (test->settings->idle_timeout < 1 ||
                test->settings->idle_timeout > MAX_TIME) {
        ...
        case OPT_RCV_TIMEOUT:
            ...
            if (rcv_timeout_in < MIN_NO_MSG_RCVD_TIMEOUT ||
                rcv_timeout_in > MAX_TIME * SEC_TO_mS) {
                i_errno = IERCVTIMEOUT;
                return -1;
            }
            test->settings->rcv_timeout.secs = rcv_timeout_in / SEC_TO_mS;
            test->settings->rcv_timeout.usecs =
                (rcv_timeout_in % SEC_TO_mS) * mS_TO_US;
            rcv_timeout_flag = 1;
            break;
#if defined(HAVE_TCP_USER_TIMEOUT)
        case OPT_SND_TIMEOUT:
            test->settings->snd_timeout = atoi(optarg);
            if (test->settings->snd_timeout < 0 ||
                test->settings->snd_timeout > MAX_TIME * SEC_TO_mS) {
    ...
    if (test->role == 'c' && test->diskfile_name != (char*)0 &&
        test->protocol->id == Pudp) {
    ...
    */
    if (test->settings->bytes == 0 && test->settings->blocks == 0 &&
        !duration_flag && test->diskfile_name != (char*)0 &&
        test->role == 'c') {
        struct stat st;
    ...
    if (test->settings->bitrate_limit_interval != 0) {
        test->settings->bitrate_limit_stats_per_interval =
            (test->settings->bitrate_limit_interval <= test->stats_interval ?
                 1 :
                 round(test->settings->bitrate_limit_interval /
                       test->stats_interval));
    }
...
void iperf_check_total_rate(struct iperf_test* test,
                            iperf_size_t last_interval_bytes_transferred)
{
    double seconds;
    uint64_t bits_per_second;
    ...
    if (++test->bitrate_limit_last_interval_index >=
        test->settings->bitrate_limit_stats_per_interval)
        test->bitrate_limit_last_interval_index = 0;
    test->bitrate_limit_intervals_traffic_bytes
        [test->bitrate_limit_last_interval_index] =
        last_interval_bytes_transferred;

    /* Ensure that enough stats periods passed to allow averaging throughput */
    test->bitrate_limit_stats_count += 1;
    if (test->bitrate_limit_stats_count <
        test->settings->bitrate_limit_stats_per_interval)
        return;

    /* Calculating total bytes traffic to be averaged */
    for (i = 0, total_bytes = 0;
         i < test->settings->bitrate_limit_stats_per_interval; i++) {
        total_bytes += test->bitrate_limit_intervals_traffic_bytes[i];
    }

    seconds =
        test->stats_interval * test->settings->bitrate_limit_stats_per_interval;
    bits_per_second = total_bytes * 8 / seconds;
    if (test->debug) {
        iperf_printf(test,
                     "Interval %" PRIu64 " - throughput %" PRIu64
                     " bps (limit %" PRIu64 ")\n",
                     test->bitrate_limit_stats_count, bits_per_second,
    ...
    if (bits_per_second > test->settings->bitrate_limit) {
        if (iperf_get_verbose(test))
            iperf_err(test,
                      "Total throughput of %" PRIu64 " bps exceeded %" PRIu64
                      " bps limit",
                      bits_per_second, test->settings->bitrate_limit);
        test->bitrate_limit_exceeded = 1;
    }
}

int iperf_send_mt(struct iperf_stream* sp)
{
...
int iperf_recv_mt(struct iperf_stream* sp)
{
    int r;
...
static void send_timer_proc(TimerClientData client_data,
...
int iperf_create_send_timers(struct iperf_test* test)
{
    struct iperf_time now;
    struct iperf_stream* sp;
    TimerClientData cd;
    ...
    SLIST_FOREACH(sp, &test->streams, streams)
    {
        ...
        if (test->settings->rate != 0 && sp->sender) {
            cd.p = sp;
            sp->send_timer = tmr_create(NULL, send_timer_proc, cd,
...
int test_is_authorized(struct iperf_test* test)
{
    ...
    if (test->settings->authtoken) {
        ...
        time_t ts;
        int rc = decode_auth_setting(test->debug, test->settings->authtoken,
                                     test->server_rsa_private_key, &username,
                                     &password, &ts);
        if (rc) {
            return -1;
        }
        int ret = check_authentication(username, password, ts,
                                       test->server_authorized_users,
                                       test->server_skew_threshold);
        if (ret == 0) {
            if (test->debug) {
                iperf_printf(test, report_authentication_succeeded, username,
                             ts);
            }
            free(username);
            free(password);
            ...
            if (test->debug) {
                iperf_printf(test, report_authentication_failed, ret, username,
                             ts);
            }
            free(username);
            free(password);
...
int iperf_exchange_results(struct iperf_test* test)
{
    if (test->role == 'c') {
...
static int send_parameters(struct iperf_test* test)
{
    ...
    if (j == NULL) {
        ...
        if (test->settings->socket_bufsize)
            cJSON_AddNumberToObject(j, "window",
        ...
        if (test->settings->pacing_timer)
            cJSON_AddNumberToObject(j, "pacing_timer",
        ...
        if (test->congestion_used)
            cJSON_AddStringToObject(j, "congestion_used",
                                    test->congestion_used);
        if (test->get_server_output)
            cJSON_AddNumberToObject(j, "get_server_output",
        ...
        if (test->repeating_payload)
            cJSON_AddNumberToObject(j, "repeating_payload",
                                    test->repeating_payload);
        if (test->zerocopy)
            cJSON_AddNumberToObject(j, "zerocopy", test->zerocopy);
#if defined(HAVE_DONT_FRAGMENT)
        if (test->settings->dont_fragment)
            cJSON_AddNumberToObject(j, "dont_fragment",
                                    test->settings->dont_fragment);
#endif /* HAVE_DONT_FRAGMENT */
#if defined(HAVE_SSL)
        /* Send authentication parameters */
        if (test->settings->client_username &&
            test->settings->client_password &&
            test->settings->client_rsa_pubkey) {
            int rc = encode_auth_setting(test->settings->client_username,
                                         test->settings->client_password,
                                         test->settings->client_rsa_pubkey,
...
#endif //HAVE_SSL
        if (test->mode && test->protocol->id == Ptcp &&
            has_tcpinfo_retransmits())
...
static int send_results(struct iperf_test* test)
{
    ...
    if (j == NULL) {
        ...
        cJSON_AddNumberToObject(j, "sender_has_retransmits",
        ...
        if (test->role == 's' && test->get_server_output) {
            if (test->json_output) {
                /* Add JSON output */
                cJSON_AddItemReferenceToObject(j, "server_output_json",
                                               test->json_top);
            }
            else {
                /* Add textual output */
        ...
        if (j_streams == NULL) {
            ...
            SLIST_FOREACH(sp, &test->streams, streams)
            {
                ...
                if (j_stream == NULL) {
                    ...
                    bytes_transferred = sp->sender ?
                                            (sp->result->bytes_sent -
                                             sp->result->bytes_sent_omit) :
                                            sp->result->bytes_received;
                    retransmits = (sp->sender && test->sender_has_retransmits) ?
                                      sp->result->stream_retrans :
                                      -1;
                    cJSON_AddNumberToObject(j_stream, "id", sp->id);
                    cJSON_AddNumberToObject(j_stream, "bytes",
                                            bytes_transferred);
                    cJSON_AddNumberToObject(j_stream, "retransmits",
                                            retransmits);
                    cJSON_AddNumberToObject(j_stream, "jitter", sp->jitter);
                    cJSON_AddNumberToObject(j_stream, "errors", sp->cnt_error);
                    cJSON_AddNumberToObject(j_stream, "omitted_errors",
                                            sp->omitted_cnt_error);
                    cJSON_AddNumberToObject(j_stream, "packets",
                                            sp->packet_count);
                    cJSON_AddNumberToObject(j_stream, "omitted_packets",
                                            sp->omitted_packet_count);

                    iperf_time_diff(&sp->result->start_time,
                                    &sp->result->start_time, &temp_time);
                    start_time = iperf_time_in_secs(&temp_time);
                    iperf_time_diff(&sp->result->start_time,
                                    &sp->result->end_time, &temp_time);
...
static int get_results(struct iperf_test* test)
{
    ...
    if (j == NULL) {
        ...
        j_sender_has_retransmits =
            cJSON_GetObjectItem(j, "sender_has_retransmits");
        if (j_cpu_util_total == NULL || j_cpu_util_user == NULL ||
            j_cpu_util_system == NULL || j_sender_has_retransmits == NULL) {
            i_errno = IERECVRESULTS;
            r = -1;
        }
        else {
            ...
            if (j_streams == NULL) {
                ...
                for (i = 0; i < n; ++i) {
                    ...
                    if (j_stream == NULL) {
                        ...
                        j_retransmits =
                            cJSON_GetObjectItem(j_stream, "retransmits");
                        j_jitter = cJSON_GetObjectItem(j_stream, "jitter");
                        j_errors = cJSON_GetObjectItem(j_stream, "errors");
                        j_omitted_errors =
                            cJSON_GetObjectItem(j_stream, "omitted_errors");
                        j_packets = cJSON_GetObjectItem(j_stream, "packets");
                        j_omitted_packets =
                            cJSON_GetObjectItem(j_stream, "omitted_packets");
                        j_start_time =
                            cJSON_GetObjectItem(j_stream, "start_time");
                        j_end_time = cJSON_GetObjectItem(j_stream, "end_time");
                        if (j_id == NULL || j_bytes == NULL ||
                            j_retransmits == NULL || j_jitter == NULL ||
                            j_errors == NULL || j_packets == NULL) {
                            i_errno = IERECVRESULTS;
                            r = -1;
                        }
                        else if ((j_omitted_errors == NULL &&
                                  j_omitted_packets != NULL) ||
                                 (j_omitted_errors != NULL &&
                                  j_omitted_packets == NULL)) {
				     ...
				     */
                                    if (j_start_time && j_end_time) {
                                        sp->result->receiver_time =
                                            j_end_time->valuedouble -
                                            j_start_time->valuedouble;
                                    }
                                    else {
                                    ...
                                    if (j_omitted_packets != NULL) {
                                        ...
                                        sp->peer_omitted_packet_count =
                                            sp->peer_packet_count;
                                    }
                                    if (j_start_time && j_end_time) {
                                        sp->result->sender_time =
                                            j_end_time->valuedouble -
                                            j_start_time->valuedouble;
                                    }
                                    else {
		 ...
		 */
                if (test->role == 'c' &&
                    ...
                    j_server_output =
                        cJSON_DetachItemFromObject(j, "server_output_json");
                    if (j_server_output != NULL) {
                        ...
                        if (j_server_output != NULL) {
                            test->server_output_text =
                                strdup(j_server_output->valuestring);
                        }
                    }
                }
            }
        ...
        if (j_remote_congestion_used != NULL) {
            test->remote_congestion_used =
                strdup(j_remote_congestion_used->valuestring);
        }
...
void add_to_interval_list(struct iperf_stream_result* rp,
                          struct iperf_interval_results* new)
{
...
void connect_msg(struct iperf_stream* sp)
{
    ...
    if (getsockdomain(sp->socket) == AF_INET) {
        inet_ntop(AF_INET,
                  (void*)&((struct sockaddr_in*)&sp->local_addr)->sin_addr, ipl,
                  sizeof(ipl));
        mapped_v4_to_regular_v4(ipl);
        inet_ntop(AF_INET,
                  (void*)&((struct sockaddr_in*)&sp->remote_addr)->sin_addr,
                  ipr, sizeof(ipr));
        ...
        inet_ntop(AF_INET6,
                  (void*)&((struct sockaddr_in6*)&sp->local_addr)->sin6_addr,
                  ipl, sizeof(ipl));
        mapped_v4_to_regular_v4(ipl);
        inet_ntop(AF_INET6,
                  (void*)&((struct sockaddr_in6*)&sp->remote_addr)->sin6_addr,
                  ipr, sizeof(ipr));
        mapped_v4_to_regular_v4(ipr);
    ...
    if (sp->test->json_output)
        cJSON_AddItemToArray(
            sp->test->json_connected,
            iperf_json_printf(
                "socket: %d  local_host: %s  local_port: %d  remote_host: %s  remote_port: %d",
                (int64_t)sp->socket, ipl, (int64_t)lport, ipr, (int64_t)rport));
    else
        iperf_printf(sp->test, report_connected, sp->socket, ipl, lport, ipr,
                     rport);
}
...
struct iperf_test* iperf_new_test()
{
    ...
    test->settings =
        (struct iperf_settings*)malloc(sizeof(struct iperf_settings));
    if (!test->settings) {
        free(test);
        i_errno = IENEWTEST;
        return NULL;
    ...
    test->bitrate_limit_intervals_traffic_bytes =
        (iperf_size_t*)malloc(sizeof(iperf_size_t) * MAX_INTERVAL);
...
int iperf_defaults(struct iperf_test* testp)
{
    ...
    testp->settings->rcv_timeout.usecs =
        (DEFAULT_NO_MSG_RCVD_TIMEOUT % SEC_TO_mS) * mS_TO_US;
    testp->zerocopy = 0;
...
void iperf_stats_callback(struct iperf_test* test)
{
    ...
    {
        rp = sp->result;
        temp.bytes_transferred = sp->sender ? rp->bytes_sent_this_interval :
                                              rp->bytes_received_this_interval;

        // Total bytes transferred this interval
        total_interval_bytes_transferred +=
            rp->bytes_sent_this_interval + rp->bytes_received_this_interval;

        irp = TAILQ_LAST(&rp->interval_results, irlisthead);
        /* result->end_time contains timestamp of previous interval */
        if (irp != NULL) /* not the 1st interval */
            memcpy(&temp.interval_start_time, &rp->end_time,
                   sizeof(struct iperf_time));
        else /* or use timestamp from beginning */
            memcpy(&temp.interval_start_time, &rp->start_time,
                   sizeof(struct iperf_time));
        /* now save time of end of this interval */
        iperf_time_now(&rp->end_time);
        memcpy(&temp.interval_end_time, &rp->end_time,
               sizeof(struct iperf_time));
        iperf_time_diff(&temp.interval_start_time, &temp.interval_end_time,
                        &temp_time);
        ...
        if (test->protocol->id == Ptcp) {
            if (has_tcpinfo()) {
                ...
                if (test->sender_has_retransmits == 1) {
                    ...
                    temp.interval_retrans =
                        total_retrans - rp->stream_prev_total_retrans;
            ...
            if (irp == NULL) {
                ...
                temp.interval_packet_count =
                    sp->packet_count - irp->packet_count;
                temp.interval_outoforder_packets =
                    sp->outoforder_packets - irp->outoforder_packets;
...
static void iperf_print_intermediate(struct iperf_test* test)
{
     ...
     */
    discard_json = test->json_stream == 1 &&
                   !(test->role == 's' && test->get_server_output);
    ...
    for (current_mode = lower_mode; current_mode <= upper_mode;
         ++current_mode) {
        char ubuf[UNIT_LEN];
        char nbuf[UNIT_LEN];
        char mbuf[UNIT_LEN];
        ...
        if (test->mode == BIDIRECTIONAL) {
            sprintf(mbuf, "[%s-%s]", stream_must_be_sender ? "TX" : "RX",
                    test->role == 'c' ? "C" : "S");
        }
        else {
            mbuf[0] = '\0';
        ...
        if (test->num_streams > 1 || test->json_output) {
            ...
            if (sp) {
                irp =
                    TAILQ_LAST(&sp->result->interval_results,
                               irlisthead); /* use 1st stream for timing info */

                unit_snprintf(ubuf, UNIT_LEN, (double)bytes, 'A');
                bandwidth = (double)bytes / (double)irp->interval_duration;
                unit_snprintf(nbuf, UNIT_LEN, bandwidth,
                              test->settings->unit_format);

                iperf_time_diff(&sp->result->start_time,
                                &irp->interval_start_time, &temp_time);
                start_time = iperf_time_in_secs(&temp_time);
                iperf_time_diff(&sp->result->start_time,
                    ...
                    if (test->sender_has_retransmits == 1 &&
                        ...
                        if (test->json_output)
                            cJSON_AddItemToObject(
                                ...
                                iperf_json_printf(
                                    "start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d  omitted: %b sender: %b",
                                    (double)start_time, (double)end_time,
                                    (double)irp->interval_duration,
                                    (int64_t)bytes, bandwidth * 8,
                                    (int64_t)retransmits, irp->omitted,
                                    stream_must_be_sender)); /* XXX irp->omitted or test->omitting? */
                        else
                            iperf_printf(
                                test, report_sum_bw_retrans_format, mbuf,
                                start_time, end_time, ubuf, nbuf, retransmits,
                                irp->omitted ?
                                    report_omitted :
                                    ""); /* XXX irp->omitted or test->omitting? */
                    }
                    else {
                        ...
                        if (test->json_output)
                            cJSON_AddItemToObject(
                                json_interval, sum_name,
                                iperf_json_printf(
                                    "start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  omitted: %b sender: %b",
                                    (double)start_time, (double)end_time,
                                    (double)irp->interval_duration,
                                    (int64_t)bytes, bandwidth * 8,
                                    test->omitting, stream_must_be_sender));
                        else
                            iperf_printf(test, report_sum_bw_format, mbuf,
                                         start_time, end_time, ubuf, nbuf,
                                         test->omitting ? report_omitted : "");
                    }
                }
                else {
                    /* Interval sum, UDP. */
                    if (stream_must_be_sender) {
                        if (test->json_output)
                            cJSON_AddItemToObject(
                                json_interval, sum_name,
                                iperf_json_printf(
                                    "start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  packets: %d  omitted: %b sender: %b",
                                    (double)start_time, (double)end_time,
                                    (double)irp->interval_duration,
                                    (int64_t)bytes, bandwidth * 8,
                                    (int64_t)total_packets, test->omitting,
                                    stream_must_be_sender));
                        else
                            iperf_printf(test, report_sum_bw_udp_sender_format,
                                         mbuf, start_time, end_time, ubuf, nbuf,
                                         zbuf, total_packets,
                                         test->omitting ? report_omitted : "");
                    }
                    else {
                        ...
                        if (test->json_output)
                            cJSON_AddItemToObject(
                                json_interval, sum_name,
                                iperf_json_printf(
                                    "start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  jitter_ms: %f  lost_packets: %d  packets: %d  lost_percent: %f  omitted: %b sender: %b",
                                    (double)start_time, (double)end_time,
                                    (double)irp->interval_duration,
                                    (int64_t)bytes, bandwidth * 8,
                                    (double)avg_jitter * 1000.0,
                                    (int64_t)lost_packets,
                                    (int64_t)total_packets,
                                    (double)lost_percent, test->omitting,
                                    stream_must_be_sender));
                        else
                            iperf_printf(test, report_sum_bw_udp_format, mbuf,
                                         start_time, end_time, ubuf, nbuf,
                                         avg_jitter * 1000.0, lost_packets,
                                         total_packets, lost_percent,
                                         test->omitting ? report_omitted : "");
                    }
                }
            }
 ...
 */
static void iperf_print_results(struct iperf_test* test)
{
    ...
    if (test->json_output) {
        ...
        if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
            if (test->sender_has_retransmits ||
                test->other_side_has_retransmits) {
                if (test->bidirectional)
    ...
    for (current_mode = lower_mode; current_mode <= upper_mode;
        ...
        int64_t total_packets = 0, lost_packets = 0;
        int64_t sender_packet_count = 0,
                receiver_packet_count = 0; /* for this stream, this interval */
        int64_t sender_omitted_packet_count = 0,
                receiver_omitted_packet_count =
                    0; /* for this stream, this interval */
        int64_t sender_total_packets = 0,
        ...
        if (sp) {
            iperf_time_diff(&sp->result->start_time, &sp->result->end_time,
                            &temp_time);
            end_time = iperf_time_in_secs(&temp_time);
            if (sp->sender) {
                ...
                if (sp->sender == stream_must_be_sender) {
                    if (test->json_output) {
                        ...
                        if (json_summary_stream == NULL)
                            return;
                        cJSON_AddItemToArray(json_summary_streams,
                                             json_summary_stream);
                    }
                     ...
                     */
                        int64_t packet_count = sender_packet_count ?
                                                   sender_packet_count :
                                                   receiver_packet_count;
                        total_packets +=
                            (packet_count - sp->omitted_packet_count);
                        sender_total_packets +=
                            (sender_packet_count - sender_omitted_packet_count);
                    ...
                    if (test->protocol->id == Ptcp ||
                        ...
                        if (test->sender_has_retransmits) {
                            ...
                            if (test->json_output)
                                cJSON_AddItemToObject(
                                    ...
                                    iperf_json_printf(
                                        "socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d  max_snd_cwnd:  %d  max_snd_wnd:  %d  max_rtt:  %d  min_rtt:  %d  mean_rtt:  %d sender: %b",
                                ...
                                iperf_printf(test, report_bw_retrans_format,
                                             sp->socket, mbuf, start_time,
                                             sender_time, ubuf, nbuf,
                                             sp->result->stream_retrans,
                                             report_sender);
                            }
                        }
                            ...
                            if (test->json_output)
                                cJSON_AddItemToObject(
                                    json_summary_stream, report_sender,
                                    iperf_json_printf(
                                        "socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f sender: %b",
                                        (int64_t)sp->socket, (double)start_time,
                                        (double)sender_time,
                                        (double)sender_time,
                                        (int64_t)bytes_sent, bandwidth * 8,
                                ...
                                iperf_printf(test, report_bw_format, sp->socket,
                                             mbuf, start_time, sender_time,
                                             ubuf, nbuf, report_sender);
                            }
                        }
                    }
                    else {
                         ...
                         */
                            int64_t packet_count = sender_packet_count ?
                                                       ...
                                                       receiver_packet_count;
                            cJSON_AddItemToObject(
                                json_summary_stream, "udp",
                                iperf_json_printf(
                                    "socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  jitter_ms: %f  lost_packets: %d  packets: %d  lost_percent: %f  out_of_order: %d sender: %b",
                                    (int64_t)sp->socket, (double)start_time,
                                    (double)sender_time, (double)sender_time,
                                    (int64_t)bytes_sent, bandwidth * 8,
                                    (double)sp->jitter * 1000.0,
                                    (int64_t)(sp->cnt_error -
                                              sp->omitted_cnt_error),
                                    (int64_t)(packet_count -
                                              sp->omitted_packet_count),
                                    (double)lost_percent,
                                    (int64_t)(sp->outoforder_packets -
                    ...
                    if (sp->diskfile_fd >= 0) {
                        if (fstat(sp->diskfile_fd, &sb) == 0) {
                            ...
                            if (sb.st_size > 0) {
                                percent_sent = (int)(((double)bytes_sent /
                                                      (double)sb.st_size) *
                                                     100.0);
                                percent_received =
                            ...
                            unit_snprintf(sbuf, UNIT_LEN, (double)sb.st_size,
                                          'A');
                            if (test->json_output)
                                cJSON_AddItemToObject(
                                    json_summary_stream, "diskfile",
                                    iperf_json_printf(
                                        "sent: %d  received: %d  size: %d  percent_sent: %d  percent_received: %d  filename: %s",
                                        (int64_t)bytes_sent,
                                        (int64_t)bytes_received,
                                        (int64_t)sb.st_size,
                                        (int64_t)percent_sent,
                                        (int64_t)percent_received,
                                        test->diskfile_name));
                            else if (stream_must_be_sender) {
                                iperf_printf(test, report_diskfile, ubuf, sbuf,
                                             percent_sent, test->diskfile_name);
                            }
                            else {
                                unit_snprintf(ubuf, UNIT_LEN,
                                              (double)bytes_received, 'A');
                                iperf_printf(test, report_diskfile, ubuf, sbuf,
                                             percent_received,
                                             test->diskfile_name);
                            }
                    ...
                    if (receiver_time > 0) {
                        bandwidth =
                            (double)bytes_received / (double)receiver_time;
                    }
                    else {
                        bandwidth = 0.0;
                    }
                    ...
                    if (test->protocol->id == Ptcp ||
                        ...
                        if (test->json_output)
                            cJSON_AddItemToObject(
                                json_summary_stream, report_receiver,
                                iperf_json_printf(
                                    "socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f sender: %b",
                                    (int64_t)sp->socket, (double)start_time,
                                    (double)receiver_time, (double)end_time,
                                    (int64_t)bytes_received, bandwidth * 8,
                                    stream_must_be_sender));
                        else if (test->role == 's' && sp->sender) {
                            if (test->verbose)
                                iperf_printf(
                                    test, report_receiver_not_available_format,
                                    sp->socket);
                        }
                        else {
                            iperf_printf(test, report_bw_format, sp->socket,
                                         mbuf, start_time, receiver_time, ubuf,
                                         nbuf, report_receiver);
                        }
                    }
                    else {
                        /*
                     ...
                     */
                        if (!test->json_output) {
                            if (receiver_packet_count -
                                        receiver_omitted_packet_count >
                                    0 &&
                                sp->omitted_cnt_error > -1) {
                                lost_percent =
                                    100.0 *
                                    (sp->cnt_error - sp->omitted_cnt_error) /
                                    (receiver_packet_count -
                            ...
                            if (test->role == 's' && sp->sender) {
                                ...
                                if (sp->omitted_cnt_error > -1) {
                                    iperf_printf(
                                        test, report_bw_udp_format, sp->socket,
                                        mbuf, start_time, receiver_time, ubuf,
                                        nbuf, sp->jitter * 1000.0,
                                        (sp->cnt_error - sp->omitted_cnt_error),
                                        (receiver_packet_count -
                                         receiver_omitted_packet_count),
        ...
        if (test->num_streams > 1 || test->json_output) {
            ...
            if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
                if (test->sender_has_retransmits) {
                    ...
                    if (test->json_output)
                        cJSON_AddItemToObject(
                            test->json_end, sum_sent_name,
                            iperf_json_printf(
                                "start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d sender: %b",
                                (double)start_time, (double)sender_time,
                                (double)sender_time, (int64_t)total_sent,
                                bandwidth * 8, (int64_t)total_retransmits,
                                stream_must_be_sender));
                    else if (test->role == 's' && !stream_must_be_sender) {
                        if (test->verbose)
                            iperf_printf(
                                test,
                        ...
                        iperf_printf(test, report_sum_bw_retrans_format, mbuf,
                                     start_time, sender_time, ubuf, nbuf,
                                     total_retransmits, report_sender);
                    }
                }
                else {
                    ...
                    if (test->json_output)
                        cJSON_AddItemToObject(
                            test->json_end, sum_sent_name,
                            iperf_json_printf(
                                "start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f sender: %b",
                                (double)start_time, (double)sender_time,
                                (double)sender_time, (int64_t)total_sent,
                ...
                if (test->json_output)
                    cJSON_AddItemToObject(
                        ...
                        iperf_json_printf(
                            "start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f sender: %b",
                            (double)start_time, (double)receiver_time,
                            (double)receiver_time, (int64_t)total_received,
                            bandwidth * 8, stream_must_be_sender));
                else if (test->role == 's' && stream_must_be_sender) {
                    if (test->verbose)
                        iperf_printf(
                            test, report_receiver_not_available_summary_format,
                            "SUM");
                }
                else {
                    iperf_printf(test, report_sum_bw_format, mbuf, start_time,
                                 receiver_time, ubuf, nbuf, report_receiver);
                }
            }
            else {
                /* Summary sum, UDP. */
                ...
                if (test->json_output) {
                     ...
                     */
                    cJSON_AddItemToObject(
                        test->json_end, sum_name,
                        iperf_json_printf(
                            "start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  jitter_ms: %f  lost_packets: %d  packets: %d  lost_percent: %f sender: %b",
                            (double)start_time, (double)receiver_time,
                            (double)receiver_time, (int64_t)total_sent,
                            bandwidth * 8, (double)avg_jitter * 1000.0,
                            (int64_t)lost_packets, (int64_t)total_packets,
                            (double)lost_percent, stream_must_be_sender));
                    /*
                    ...
                    cJSON_AddItemToObject(
                        ...
                        iperf_json_printf(
                            ...
                            (double)start_time, (double)sender_time,
                            (double)sender_time, (int64_t)total_sent,
                            (double)total_sent * 8 / sender_time, (double)0.0,
                            (int64_t)0, (int64_t)sender_total_packets,
                            (double)0.0, 1));
                    cJSON_AddItemToObject(
                        test->json_end, sum_received_name,
                        iperf_json_printf(
                            ...
                            (double)start_time, (double)receiver_time,
                            (double)receiver_time, (int64_t)total_received,
                            (double)total_received * 8 / receiver_time,
                            (double)avg_jitter * 1000.0, (int64_t)lost_packets,
                            (int64_t)receiver_total_packets,
                    ...
                    if (!(test->role == 's' && !stream_must_be_sender)) {
                        unit_snprintf(ubuf, UNIT_LEN, (double)total_sent, 'A');
                        iperf_printf(test, report_sum_bw_udp_format, mbuf,
                                     start_time, sender_time, ubuf, nbuf, 0.0,
                                     (int64_t)0, sender_total_packets, 0.0,
                                     report_sender);
                    }
                    if (!(test->role == 's' && stream_must_be_sender)) {
                        unit_snprintf(ubuf, UNIT_LEN, (double)total_received,
                                      'A');
                        /* Compute received bandwidth. */
                        if (end_time > 0.0) {
                            bandwidth =
                                (double)total_received / (double)receiver_time;
                        }
                        else {
                            bandwidth = 0.0;
                        }
                        unit_snprintf(nbuf, UNIT_LEN, bandwidth,
                                      test->settings->unit_format);
                        iperf_printf(test, report_sum_bw_udp_format, mbuf,
                                     start_time, receiver_time, ubuf, nbuf,
                                     avg_jitter * 1000.0, lost_packets,
                                     receiver_total_packets, lost_percent,
                                     report_receiver);
                    }
                }
            }
        ...
        if (test->json_output && current_mode == upper_mode) {
            cJSON_AddItemToObject(
                test->json_end, "cpu_utilization_percent",
                iperf_json_printf(
                    "host_total: %f  host_user: %f  host_system: %f  remote_total: %f  remote_user: %f  remote_system: %f",
                    (double)test->cpu_util[0], (double)test->cpu_util[1],
                    (double)test->cpu_util[2], (double)test->remote_cpu_util[0],
                    (double)test->remote_cpu_util[1],
                    (double)test->remote_cpu_util[2]));
            if (test->protocol->id == Ptcp) {
                ...
                if (snd_congestion) {
                    cJSON_AddStringToObject(test->json_end,
            ...
            if (test->verbose) {
                if (stream_must_be_sender) {
                    if (test->bidirectional) {
                        iperf_printf(test, report_cpu, report_local,
                                     stream_must_be_sender ? report_sender :
                                                             report_receiver,
                                     test->cpu_util[0], test->cpu_util[1],
                                     test->cpu_util[2], report_remote,
                                     stream_must_be_sender ? report_receiver :
                                                             report_sender,
                                   ...
                                   cJSON* json_interval_streams)
{
    ...
    if (test->mode == BIDIRECTIONAL) {
        sprintf(mbuf, "[%s-%s]", sp->sender ? "TX" : "RX",
                test->role == 'c' ? "C" : "S");
    }
    else {
        mbuf[0] = '\0';
    ...
    irp = TAILQ_LAST(&sp->result->interval_results,
                     irlisthead); /* get last entry in linked list */
    if (irp == NULL) {
        iperf_err(test,
                  "print_interval_results error: interval_results is NULL");
        return;
    }
    if (!test->json_output) {
        /* First stream? */
	    ...
	    ** else nothing.
	    */
            if (iperf_time_compare(&sp->result->start_time,
                                   &irp->interval_start_time) == 0) {
                if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
                    if (test->sender_has_retransmits == 1) {
                        if (test->bidirectional)
                            iperf_printf(test, "%s",
    ...
    if (irp->interval_duration > 0.0) {
        bandwidth =
            (double)irp->bytes_transferred / (double)irp->interval_duration;
    }
    else {
        bandwidth = 0.0;
    }
    ...
    iperf_time_diff(&sp->result->start_time, &irp->interval_start_time,
                    &temp_time);
    st = iperf_time_in_secs(&temp_time);
    iperf_time_diff(&sp->result->start_time, &irp->interval_end_time,
                    &temp_time);
        ...
        if (test->sender_has_retransmits == 1 && sp->sender) {
            ...
            if (test->json_output)
                cJSON_AddItemToArray(
                    ...
                    iperf_json_printf(
                        "socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d  snd_cwnd:  %d  snd_wnd:  %d  rtt:  %d  rttvar: %d  pmtu: %d  omitted: %b sender: %b",
                        (int64_t)sp->socket, (double)st, (double)et,
                        (double)irp->interval_duration,
                        (int64_t)irp->bytes_transferred, bandwidth * 8,
                        (int64_t)irp->interval_retrans, (int64_t)irp->snd_cwnd,
                        (int64_t)irp->snd_wnd, (int64_t)irp->rtt,
                        (int64_t)irp->rttvar, (int64_t)irp->pmtu, irp->omitted,
                        sp->sender));
            else {
                unit_snprintf(cbuf, UNIT_LEN, irp->snd_cwnd, 'A');
                iperf_printf(test, report_bw_retrans_cwnd_format, sp->socket,
                             mbuf, st, et, ubuf, nbuf, irp->interval_retrans,
                             cbuf, irp->omitted ? report_omitted : "");
            }
        }
        else {
            ...
            if (test->json_output)
                cJSON_AddItemToArray(
                    json_interval_streams,
                    iperf_json_printf(
                        "socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  omitted: %b sender: %b",
                        (int64_t)sp->socket, (double)st, (double)et,
                        (double)irp->interval_duration,
                        (int64_t)irp->bytes_transferred, bandwidth * 8,
                        irp->omitted, sp->sender));
            else
                iperf_printf(test, report_bw_format, sp->socket, mbuf, st, et,
                             ubuf, nbuf, irp->omitted ? report_omitted : "");
        }
    }
    else {
        /* Interval, UDP. */
        if (sp->sender) {
            if (test->json_output)
                cJSON_AddItemToArray(
                    json_interval_streams,
                    iperf_json_printf(
                        "socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  packets: %d  omitted: %b sender: %b",
                        (int64_t)sp->socket, (double)st, (double)et,
                        (double)irp->interval_duration,
                        (int64_t)irp->bytes_transferred, bandwidth * 8,
                        (int64_t)irp->interval_packet_count, irp->omitted,
                        sp->sender));
            else
                iperf_printf(test, report_bw_udp_sender_format, sp->socket,
                             mbuf, st, et, ubuf, nbuf, zbuf,
            ...
            if (test->json_output)
                cJSON_AddItemToArray(
                    json_interval_streams,
                    iperf_json_printf(
                        "socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  jitter_ms: %f  lost_packets: %d  packets: %d  lost_percent: %f  omitted: %b sender: %b",
                        (int64_t)sp->socket, (double)st, (double)et,
                        (double)irp->interval_duration,
                        (int64_t)irp->bytes_transferred, bandwidth * 8,
                        (double)irp->jitter * 1000.0,
                        (int64_t)irp->interval_cnt_error,
                        (int64_t)irp->interval_packet_count,
                        (double)lost_percent, irp->omitted, sp->sender));
            else
                iperf_printf(test, report_bw_udp_format, sp->socket, mbuf, st,
                             et, ubuf, nbuf, irp->jitter * 1000.0,
                             irp->interval_cnt_error,
                             irp->interval_packet_count, lost_percent,
                             irp->omitted ? report_omitted : "");
        }
    }
                                      ...
                                      int sender)
{
    ...
    sp->result =
        (struct iperf_stream_result*)malloc(sizeof(struct iperf_stream_result));
    if (!sp->result) {
        free(sp);
        i_errno = IECREATESTREAM;
        return NULL;
    ...
    sp->buffer = (char*)mmap(NULL, test->settings->blksize,
                             PROT_READ | PROT_WRITE, MAP_PRIVATE, sp->buffer_fd,
                             0);
...
/**************************************************************************/
int iperf_common_sockopts(struct iperf_test* test, int s)
{
    int opt;
...
int iperf_init_stream(struct iperf_stream* sp, struct iperf_test* test)
{
    ...
    if (iperf_get_test_protocol_id(test) == Pudp &&
        getsockdomain(sp->socket) == AF_INET && iperf_get_dont_fragment(test)) {
        /*
        ...
        opt = IP_PMTUDISC_DO;
        if (setsockopt(sp->socket, IPPROTO_IP, IP_MTU_DISCOVER, &opt,
                       sizeof(opt)) < 0) {
        ...
        opt = 1;
        if (setsockopt(sp->socket, IPPROTO_IP, IP_DONTFRAG, &opt, sizeof(opt)) <
            0) {
            i_errno = IESETDONTFRAGMENT;
            return -1;
        }
        ...
        opt = 1;
        if (setsockopt(sp->socket, IPPROTO_IP, IP_DONTFRAGMENT, &opt,
                       sizeof(opt)) < 0) {
            i_errno = IESETDONTFRAGMENT;
            return -1;
        }
    ...
    if (test->pidfile) {
        ...
        if (fd >= 0) {
			 ...
			 */
                        ...
                        iperf_errexit(
                            test,
                            "Another instance of iperf3 appears to be running");
                    }
                }
            }
        }
...
int iperf_delete_pidfile(struct iperf_test* test)
{
    if (test->pidfile) {
...
int iperf_json_finish(struct iperf_test* test)
{
    if (test->json_top) {
        ...
        if (test->extra_data) {
            cJSON_AddStringToObject(test->json_top, "extra_data",
                                    test->extra_data);
        }
    ...
    if (test->json_stream) {
        ...
        if (test->json_server_output) {
            JSONStream_Output(test, "server_output_json",
                              test->json_server_output);
        }
        if (test->server_output_text) {
            JSONStream_Output(test, "server_output_text",
                              cJSON_CreateString(test->server_output_text));
        }
    ...
    cJSON_Delete(test->json_top);
    test->json_top = test->json_start = test->json_connected =
        test->json_intervals = test->json_server_output = test->json_end = NULL;
    return 0;
}

/* CPU affinity stuff - Linux, FreeBSD, and Windows only. */

int iperf_setaffinity(struct iperf_test* test, int affinity)
{
    ...
    if (cpuset_getaffinity(CPU_LEVEL_WHICH, CPU_WHICH_PID, -1, sizeof(cpuset_t),
                           &test->cpumask) != 0) {
        i_errno = IEAFFINITY;
        return -1;
    }
...
int iperf_clearaffinity(struct iperf_test* test)
{
#if defined(HAVE_SCHED_SETAFFINITY)
    cpu_set_t cpu_set;
    int i;
...
int iperf_printf(struct iperf_test* test, const char* format, ...)
{
    ...
    if (iperf_get_test_timestamps(test)) {
        time(&now);
        ltm = localtime(&now);
        strftime(iperf_timestr, sizeof(iperf_timestr),
    ...
    if (test->role == 'c') {
        ...
        if (r < sizeof(linebuffer)) {
            va_start(argp, format);
            r0 =
                vsnprintf(linebuffer + r, sizeof(linebuffer) - r, format, argp);
            va_end(argp);
            if (r0 < 0) {
                r = r0;
                goto bottom;
        ...
        if (test->role == 's' && iperf_get_test_get_server_output(test)) {
            struct iperf_textline* l =
                (struct iperf_textline*)malloc(sizeof(struct iperf_textline));
