...
void iperf_err(struct iperf_test* test, const char* format, ...)
{
    ...
    if (test != NULL && test->timestamps) {
        time(&now);
        ltm = localtime(&now);
        strftime(iperf_timestrerr, sizeof(iperf_timestrerr),
                 test->timestamp_format, ltm);
        ct = iperf_timestrerr;
    }
...
char* iperf_strerror(int int_errno)
{
    ...
    case IECLIENTONLY:
        ...
        break;
    case IEDURATION:
        snprintf(errstr, len, "test duration too long (maximum = %d seconds)",
                 MAX_TIME);
        break;
    case IENUMSTREAMS:
        snprintf(errstr, len,
                 "number of parallel streams too large (maximum = %d)",
                 MAX_STREAMS);
        break;
    case IEBLOCKSIZE:
    ...
    case IEBUFSIZE:
        snprintf(errstr, len,
                 "socket buffer size too large (maximum = %d bytes)",
                 MAX_TCP_BUFFER);
        break;
    case IEINTERVAL:
        snprintf(errstr, len,
                 "invalid report interval (min = %g, max = %g seconds)",
    ...
    case IEUDPBLOCKSIZE:
        snprintf(errstr, len,
                 "block size invalid (minimum = %d bytes, maximum = %d bytes)",
                 MIN_UDP_BLOCKSIZE, MAX_UDP_BLOCKSIZE);
        break;
    case IEBADTOS:
        snprintf(errstr, len,
                 "bad TOS value (must be between 0 and 255 inclusive)");
        break;
    ...
    case IEBADFORMAT:
        snprintf(
            errstr, len,
            "bad format specifier (valid formats are in the set [kmgtKMGT])");
        break;
    case IEBADPORT:
        snprintf(errstr, len,
    ...
    case IEUNIMP:
        snprintf(errstr, len,
                 "an option you are trying to set is not implemented yet");
        break;
    case IEFILE:
    ...
    case IEENDCONDITIONS:
        snprintf(errstr, len,
                 "only one test end condition (-t, -n, -k) may be specified");
        break;
    case IELOGFILE:
    ...
    case IERECVMESSAGE:
        snprintf(
            errstr, len,
            "unable to receive control message - port may not be available, the other side may have stopped running, etc.");
        perr = 1;
        break;
    case IESENDPARAMS:
    ...
    case IESNDTIMEOUT:
        snprintf(errstr, len,
                 "send timeout value is incorrect or not in range");
        perr = 1;
        break;
    ...
    case IEBINDDEV:
        snprintf(errstr, len,
                 "Unable to bind-to-device (check perror, maybe permissions?)");
        break;
    case IEBINDDEVNOSUPPORT:
        snprintf(
            errstr, len,
            "`<ip>%%<dev>` is not supported as system does not support bind to device");
        break;
    case IEHOSTDEV:
        snprintf(
            errstr, len,
            "host device name (ip%%<dev>) is supported (and required) only for IPv6 link-local address");
        break;
    case IENOMSG:
