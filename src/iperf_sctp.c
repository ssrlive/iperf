...
int iperf_sctp_listen(struct iperf_test* test)
{
    ...
    if (test->bind_dev) {
        ...
        if (setsockopt(s, SOL_SOCKET, SO_BINDTODEVICE, test->bind_dev,
                       IFNAMSIZ) < 0)
...
int iperf_sctp_connect(struct iperf_test* test)
{
    ...
    if (test->bind_address) {
        ...
        hints.ai_family = test->settings->domain;
        hints.ai_socktype = SOCK_STREAM;
        if ((gerror = getaddrinfo(test->bind_address, NULL, &hints,
                                  &local_res)) != 0) {
            i_errno = IESTREAMCONNECT;
            return -1;
        }
    ...
    if ((gerror = getaddrinfo(test->server_hostname, portstr, &hints,
                              &server_res)) != 0) {
        if (test->bind_address)
    ...
    if (test->bind_address) {
        ...
        if (bind(s, (struct sockaddr*)local_res->ai_addr,
                 local_res->ai_addrlen) < 0) {
            saved_errno = errno;
            close(s);
            freeaddrinfo(local_res);
            freeaddrinfo(server_res);
    ...
    if (test->settings->num_ostreams > 0) {
        ...
        if (setsockopt(s, IPPROTO_SCTP, SCTP_INITMSG, &initmsg,
     ...
     */
    opt = 0;
    if (setsockopt(s, IPPROTO_SCTP, SCTP_DISABLE_FRAGMENTS, &opt, sizeof(opt)) <
            0 &&
        errno != ENOPROTOOPT) {
        saved_errno = errno;
        close(s);
...
int iperf_sctp_bindx(struct iperf_test* test, int s, int is_server)
{
    ...
    if (!is_server) {
        ...
        if ((gerror = getaddrinfo(xbe0->name, servname, &hints, &xbe0->ai)) !=
            0) {
    ...
    TAILQ_FOREACH(xbe, &test->xbind_addrs, link)
    {
        if (xbe->ai != NULL)
            freeaddrinfo(xbe->ai);
        if ((gerror = getaddrinfo(xbe->name, servname, &hints, &xbe->ai)) !=
            0) {
            i_errno = IESETSCTPBINDX;
            retval = -1;
            goto out;
