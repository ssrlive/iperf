...
typedef unsigned int uint
    ...
    defined(TCP_INFO)
    struct tcp_info
        tcpInfo; /* getsockopt(TCP_INFO) for Linux, {Free,Net,Open}BSD */
#else
...
struct iperf_settings {
    ...
    iperf_size_t
        bitrate_limit; /* server's maximum allowed total data rate for all streams*/
...
enum iperf_mode { SENDER = 1, RECEIVER = 0, BIDIRECTIONAL = -1 };
...
struct iperf_test {
    ...
    enum debug_level
        debug_level; /* -d option option - level of debug messages to show */
    ...
    double remote_cpu_util
        [3]; /* cpu utilization for the remote host/client - total, user, system */
    ...
    iperf_size_t
        bitrate_limit_stats_count; /* Number of stats periods accumulated for server's total bitrate average */
    iperf_size_t*
        bitrate_limit_intervals_traffic_bytes; /* Pointer to a cyclic array that includes the last interval's bytes transferred */
    iperf_size_t
        bitrate_limit_last_interval_index; /* Index of the last interval traffic inserted into the cyclic array */
...
/* default settings */
#define PORT \
    5201 /* default port to listen on (don't use the same port as iperf2) */
...
extern int
    gerror; /* error value from getaddrinfo(3), for use in internal error handling */
...
#define LEGACY_UDP_CONNECT_REPLY \
    987654321 // Old servers may still reply with the legacy value
#endif
