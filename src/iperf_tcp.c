...
int iperf_tcp_send(struct iperf_stream* sp)
{
    ...
    if (sp->test->debug_level >= DEBUG_LEVEL_DEBUG)
        printf("sent %d bytes of %d, pending %d, total %" PRIu64 "\n", r,
               sp->settings->blksize, sp->pending_size, sp->result->bytes_sent);
...
int iperf_tcp_accept(struct iperf_test* test)
{
    ...
    if (strcmp(test->cookie, cookie) != 0) {
        if (Nwrite(s, (char*)&rbuf, sizeof(rbuf), Ptcp) < 0) {
            iperf_err(
                test,
                "failed to send access denied from busy server to new connecting client, errno = %d\n",
                errno);
        }
        close(s);
    }
...
int iperf_tcp_listen(struct iperf_test* test)
{
	 ...
	 */
        ...
        if ((gerror = getaddrinfo(test->bind_address, portstr, &hints, &res)) !=
            0) {
        ...
        if (test->no_delay) {
            opt = 1;
            if (setsockopt(s, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt)) <
                0) {
                saved_errno = errno;
                close(s);
                freeaddrinfo(res);
...
int iperf_tcp_connect(struct iperf_test* test)
{
    ...
    if ((opt = test->settings->snd_timeout)) {
        if (setsockopt(s, IPPROTO_TCP, TCP_USER_TIMEOUT, &opt, sizeof(opt)) <
            0) {
            saved_errno = errno;
            close(s);
            freeaddrinfo(server_res);
    ...
    if (test->settings->flowlabel) {
        if (server_res->ai_addr->sa_family != AF_INET6) {
            ...
            struct sockaddr_in6* sa6P =
                (struct sockaddr_in6*)server_res->ai_addr;
            ...
            freq->flr_label =
                htonl(test->settings->flowlabel & IPV6_FLOWINFO_FLOWLABEL);
            freq->flr_action = IPV6_FL_A_GET;
            freq->flr_flags = IPV6_FL_F_CREATE;
            freq->flr_share = IPV6_FL_S_ANY;
            memcpy(&freq->flr_dst, &sa6P->sin6_addr, 16);

            if (setsockopt(s, IPPROTO_IPV6, IPV6_FLOWLABEL_MGR, freq,
                           freq_len) < 0) {
                saved_errno = errno;
                close(s);
                freeaddrinfo(server_res);
            ...
            opt = 1;
            if (setsockopt(s, IPPROTO_IPV6, IPV6_FLOWINFO_SEND, &opt,
                           sizeof(opt)) < 0) {
                saved_errno = errno;
                close(s);
                freeaddrinfo(server_res);
