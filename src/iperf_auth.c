...
int check_authentication(const char* username, const char* password,
                         const time_t ts, const char* filename,
                         int skew_threshold)
{
    time_t t = time(NULL);
    time_t utc_seconds = mktime(localtime(&t));
    if ((utc_seconds - ts) > skew_threshold ||
        (utc_seconds - ts) < -skew_threshold) {
        return 1;
    }
    ...
    while (fgets(buf, 1024, ptr_file)) {
        ...
        if (strcmp(username, s_username) == 0 &&
            strcmp(passwordHash, s_password) == 0) {
            fclose(ptr_file);
            return 0;
        }
    }
...
int Base64Encode(const unsigned char* buffer, const size_t length,
                 char** b64text)
...
{ //Decodes a base64 encoded string
    ...
    *length = BIO_read(bio, *buffer, strlen(b64message));
    assert(
        *length ==
        decodeLen); //length should equal decodeLen, else something went horribly wrong
    BIO_free_all(bio);
...
int encrypt_rsa_message(const char* plaintext, EVP_PKEY* public_key,
                        unsigned char** encryptedtext)
{
    ...
    rc = EVP_PKEY_get_int_param(public_key, OSSL_PKEY_PARAM_MAX_SIZE,
                                &keysize); /* XXX not really keysize */
    if (!rc) {
        goto errreturn;
    }
#else
    ...
    *encryptedtext = (unsigned char*)OPENSSL_malloc(keysize);
    ...
    EVP_PKEY_encrypt(ctx, *encryptedtext, &encryptedtext_len, rsa_buffer,
                     rsa_buffer_len);
    EVP_PKEY_CTX_free(ctx);
#else
    encryptedtext_len = RSA_public_encrypt(rsa_buffer_len, rsa_buffer,
                                           *encryptedtext, rsa,
                                           RSA_PKCS1_PADDING);
    RSA_free(rsa);
#endif
...
int decrypt_rsa_message(const unsigned char* encryptedtext,
                        const int encryptedtext_len, EVP_PKEY* private_key,
                        unsigned char** plaintext)
{
    ...
    /* See evp_pkey_rsa(7) and provider-keymgmt(7) */
    rc = EVP_PKEY_get_int_param(private_key, OSSL_PKEY_PARAM_MAX_SIZE,
                                &keysize); /* XXX not really keysize */
    if (!rc) {
        goto errreturn;
    }
    ...
    *plaintext = (unsigned char*)OPENSSL_malloc(keysize);
    ...
    EVP_PKEY_decrypt(ctx, *plaintext, &plaintext_len, rsa_buffer,
                     rsa_buffer_len);
    EVP_PKEY_CTX_free(ctx);
#else
    plaintext_len = RSA_private_decrypt(rsa_buffer_len, rsa_buffer, *plaintext,
...
int encode_auth_setting(const char* username, const char* password,
                        EVP_PKEY* public_key, char** authtoken)
{
    time_t t = time(NULL);
    time_t utc_seconds = mktime(localtime(&t));
     ...
     */
    const int text_len =
        strlen(auth_text_format) + strlen(username) + strlen(password) + 32;
    char* text = (char*)calloc(text_len, sizeof(char));
    if (text == NULL) {
        return -1;
    }
    snprintf(text, text_len, auth_text_format, username, password,
             (int64_t)utc_seconds);
...
int decode_auth_setting(int enable_debug, const char* authtoken,
                        EVP_PKEY* private_key, char** username, char** password,
                        time_t* ts)
{
