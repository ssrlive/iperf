                    ...
                    int timeout)
{
    ...
    if ((ret = connect(s, name, namelen)) != 0 && errno == EINPROGRESS) {
        ...
        if ((ret = poll(&pfd, 1, timeout)) == 1) {
            ...
            if ((ret = getsockopt(s, SOL_SOCKET, SO_ERROR, &optval, &optlen)) ==
                0) {
                errno = optval;
                ret = optval == 0 ? 0 : -1;
            }
            ...
            int local_port, const char* server, int port, int timeout)
{
    ...
    s = create_socket(domain, proto, local, bind_dev, local_port, server, port,
                      &server_res);
    if (s < 0) {
        return -1;
    }

    if (timeout_connect(s, (struct sockaddr*)server_res->ai_addr,
                        server_res->ai_addrlen, timeout) < 0 &&
        errno != EINPROGRESS) {
        saved_errno = errno;
        close(s);
        freeaddrinfo(server_res);
                ...
                int port)
{
    ...
    if (res->ai_family == AF_INET6 &&
        (domain == AF_UNSPEC || domain == AF_INET6)) {
        if (domain == AF_UNSPEC)
            opt = 0;
        else
            opt = 1;
...
int Nread(int fd, char* buf, size_t count, int prot)
{
    ...
    while (nleft > 0) {
        ...
        if (nleft > 0) {
            ...
            if (ftimeout.secs == 0) {
                ftimeout = now;
                iperf_time_add_usecs(&ftimeout,
                                     nread_overall_timeout * 1000000L);
            }
