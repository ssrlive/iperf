...
int iperf_udp_recv(struct iperf_stream* sp)
{
	 ...
	 */
	     ...
	     */
            ...
            if (sp->test->debug)
                fprintf(stderr,
                        "OUT OF ORDER - incoming packet sequence %" PRIu64
                        " but expected sequence %" PRIu64 " on stream %d",
 ...
 */
int iperf_udp_buffercheck(struct iperf_test* test, int s)
{
    int rc = 0;
    ...
    if (test->settings->socket_bufsize &&
        test->settings->socket_bufsize > sndbuf_actual) {
        i_errno = IESETBUF2;
        return -1;
    }
    ...
    if (test->settings->socket_bufsize &&
        test->settings->socket_bufsize > rcvbuf_actual) {
        i_errno = IESETBUF2;
        return -1;
    }
    ...
    if (test->json_output) {
        cJSON* sock_bufsize_item =
            cJSON_GetObjectItem(test->json_start, "sock_bufsize");
        if (sock_bufsize_item == NULL) {
            cJSON_AddNumberToObject(test->json_start, "sock_bufsize",
                                    test->settings->socket_bufsize);
        }

        cJSON* sndbuf_actual_item =
            cJSON_GetObjectItem(test->json_start, "sndbuf_actual");
        if (sndbuf_actual_item == NULL) {
            cJSON_AddNumberToObject(test->json_start, "sndbuf_actual",
                                    sndbuf_actual);
        }

        cJSON* rcvbuf_actual_item =
            cJSON_GetObjectItem(test->json_start, "rcvbuf_actual");
        if (rcvbuf_actual_item == NULL) {
            cJSON_AddNumberToObject(test->json_start, "rcvbuf_actual",
                                    rcvbuf_actual);
        }
    }
...
int iperf_udp_accept(struct iperf_test* test)
{
     ...
     */
    len = sizeof(sa_peer);
    if ((sz = recvfrom(test->prot_listener, &buf, sizeof(buf), 0,
                       (struct sockaddr*)&sa_peer, &len)) < 0) {
    ...
    if (test->settings->fqrate) {
        ...
        if (fqrate > 0) {
            ...
            if (setsockopt(s, SOL_SOCKET, SO_MAX_PACING_RATE, &fqrate,
                           sizeof(fqrate)) < 0) {
    ...
    test->max_fd = (test->max_fd < test->prot_listener) ? test->prot_listener :
                                                          test->max_fd;
...
int iperf_udp_listen(struct iperf_test* test)
{
    ...
    if ((s = netannounce(test->settings->domain, Pudp, test->bind_address,
                         test->bind_dev, test->server_port)) < 0) {
        i_errno = IESTREAMLISTEN;
        return -1;
    }
...
int iperf_udp_connect(struct iperf_test* test)
{
    ...
    if (test->reverse) /* In reverse mode allow few packets to have the "accept" response - to handle out of order packets */
        max_len_wait_for_reply +=
            MAX_REVERSE_OUT_OF_ORDER_PACKETS * test->settings->blksize;
    do {
        ...
        if (test->debug) {
            printf(
                "Connect received for Socket %d, sz=%d, buf=%x, i=%d, max_len_wait_for_reply=%d\n",
                s, sz, buf, i, max_len_wait_for_reply);
        }
        i += sz;
    } while (buf != UDP_CONNECT_REPLY && buf != LEGACY_UDP_CONNECT_REPLY &&
             i < max_len_wait_for_reply);
